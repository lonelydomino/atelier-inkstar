...
export function SaleBanner() {
  ...
  return (
    ...
    >
      ...
      <div className="section-shell relative flex min-h-[4.5rem] items-center justify-center py-5 text-center sm:min-h-[5rem] sm:py-6">
        <p className="text-base font-semibold tracking-wide sm:text-lg">
          <span className="sale-banner-sparkle mr-2 text-ink-gold" aria-hidden>
            ✦
          </span>
          <span className="shimmer-text">{headline}</span>{" "}
          <span className="text-white">{datesLabel} !</span>
          <span className="sale-banner-sparkle ml-2 text-ink-gold" aria-hidden>
            ✦
          </span>
        </p>
      </div>
